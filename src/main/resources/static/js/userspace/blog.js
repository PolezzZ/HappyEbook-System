...
$(function() {
	...
	$(".blog-content-container").on("click","#submitComment", function () {
		...
		$.ajax({ 
			 url: '/comments', 
			 type: 'POST', 
			 data:{"ebookId":ebookId, "commentContent":$('#commentContent').val()},
			 beforeSend: function(request) {
                 request.setRequestHeader(csrfHeader, csrfToken); // 添加  CSRF Token 
             },
			 success: function(data){
	...
	$(".blog-content-container").on("click",".blog-delete-comment", function () { 
		...
		$.ajax({ 
			 url: '/comments/'+$(this).attr("commentId")+'?ebookId='+ebookId, 
			 type: 'DELETE', 
			 beforeSend: function(request) {
                 request.setRequestHeader(csrfHeader, csrfToken); // 添加  CSRF Token 
             },
	...
	$(".blog-content-container").on("click","#cancelVote", function () { 
		...
		$.ajax({ 
			 url: '/votes/'+$(this).attr('voteId')+'?ebookId='+ebookId, 
			 type: 'DELETE', 
			 beforeSend: function(request) {
                 request.setRequestHeader(csrfHeader, csrfToken); // 添加  CSRF Token 
             },
